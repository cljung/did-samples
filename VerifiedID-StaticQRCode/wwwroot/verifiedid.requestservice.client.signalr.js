function RequestService(onDrawQRCode, onNavigateToDeepLink, onRequestRetrieved, onPresentationVerified, onIssuanceSuccesful, onSelfieTaken, onError, pollFrequency) {
    ...
    this.onSignalRConnected = async function (connectionId) {
        console.log(`connection.start ${connectionId}`);
    }
} // RequestService
...
connection.on("StationEvent", function (stationId, id, requestStatus, error) {
    requestService.log(`SignalR - StationEvent: stationId: ${stationId}, id: ${id},  requestStatus: ${requestStatus}`);
    requestService.getRequestStatus(id, requestStatus);
});

connection.start().then(function () {
    console.log(`connection.start ${connection.connectionId}`);
    requestService.onSignalRConnected(connection.connectionId);
}).catch(function (err) {
